<tr class="hover:bg-gray-100">
    ...
    <td class="p-2 flex items-center whitespace-nowrap space-x-6 mr-12 lg:mr-0">
        <img class="h-10 w-10 rounded-full" src="https://ui-avatars.com/api/?name={{$service->name}}" alt="Michael Gough avatar">
        <div class="text-sm font-normal text-gray-500">
            <div class="text-sm font-normal text-gray-500">{{$service->name}}</div>
        </div>
    </td>
    <td class="p-2 whitespace-nowrap text-base font-medium text-gray-900">
        @if($service->desc != null)
            <div class="text-sm font-normal text-gray-500">{{$service->desc}}</div>
        @else
    ...
    <td class="p-2 whitespace-nowrap space-x-2">
        <form method="POST" action="{{ route('services.save') }}">
            <button type="button" data-modal-toggle="user-modal-{{$service->id}}" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
