<tr class="hover:bg-gray-100">
    ...
    <td class="p-2 flex items-center whitespace-nowrap space-x-6 mr-12 lg:mr-0">
        <img class="h-10 w-10 rounded-full" src="https://ui-avatars.com/api/?name={{$pole->name}}" alt="Michael Gough avatar">
        <div class="text-sm font-normal text-gray-500">
            <div class="text-sm font-normal text-gray-500">{{$pole->name}}</div>
        </div>
    </td>
    <td class="p-2 whitespace-nowrap space-x-2">
        <button type="button" data-modal-toggle="user-modal-{{$pole->id}}" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
    ...
    <!-- Modal -->
    <div id="user-modal-{{$pole->id}}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 p-2 md:inset-0 h-modal md:h-full">
