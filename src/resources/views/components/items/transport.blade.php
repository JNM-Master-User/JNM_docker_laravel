<div>
    <form method="POST" action="{{ route('transports.update') }}">
        <table class="table-fixed min-w-full divide-y divide-gray-200">
            <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-500">
            <tr class="hover:bg-gray-100">
                @csrf
                <input class="hidden" name="id" value='{{$transport->id}}'>
                <td class="p-2 w-4">
                ...
                <td class="p-2 flex items-center whitespace-nowrap space-x-6 mr-12 lg:mr-0">
                    <img class="h-10 w-10 rounded-full" src="https://ui-avatars.com/api/?name={{$transport->name}}"
                         alt="">
                ...
                <td class="p-2 whitespace-nowrap text-base font-medium text-gray-900">
                    <div id="transport-path_picture-{{$transport->id}}"
                ...
                <td class="p-2 whitespace-nowrap space-x-2">
                    ...
                    <button id="buttonSaveTransport-{{$transport->id}}" type="submit" class="hidden text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-200 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
                        <i class="mr-2 fa-lg fa-fw fa-solid fa-user-pen"></i>
                        {{__('Save Transport')}}
                    </button>
                </td>
                <td class="p-2 whitespace-nowrap space-x-2">
                    <button id="buttonEditTransport-{{$transport->id}}" type="button"
                    ...
                    <script type="text/javascript">
                        document.getElementById('buttonEditTransport-{{$transport->id}}').addEventListener('click', () => {
                            ...
                            document.getElementById('transport-name-{{$transport->id}}').innerHTML = `<input name="name" value='{{$transport->name}}' />`;
                            document.getElementById('transport-path_picture-{{$transport->id}}').innerHTML = `<input name="path_picture" value='{{$transport->path_picture}}'  />`;
                        });
                    </script>
                </td>
                <td class="p-2 whitespace-nowrap space-x-2">
                    <form method="POST" action="{{ route('transports.destroy') }}">
                        @csrf
                        <input type="hidden" name="id" value="{{$transport->id}}">
                        <button id="buttonDeleteTransport" type="submit"
